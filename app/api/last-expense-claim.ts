...
export default async function handler(
  ...
  try {
    ...
    doc
      .fontSize(14)
      .fillColor("#1a237e")
      .text("4. Approved By (Official Use)", {
        underline: true,
      });
    doc.moveDown();
    doc.fontSize(12).fillColor("black");
    ...
    doc.text(
      `Medical Assessor Signature: ${medicalAssessorSignature || "N/A"}`,
    );
    doc.moveDown();
    ...
    doc
      .fontSize(14)
      .fillColor("#1a237e")
      .text("5. Required Supporting Documents", {
        underline: true,
      });
    doc.moveDown();
    doc.fontSize(12).fillColor("black");
    ...
    doc.text(
      `Copy of Deceased’s ID: ${deceasedIDCopy ? "✓ Provided" : "✗ Missing"}`,
    );
    doc.text(
      `Copy of Claimant’s ID: ${claimantIDCopy ? "✓ Provided" : "✗ Missing"}`,
    );
    doc.text(
      `Bank / M-Pesa Details: ${bankMpesaDetails ? "✓ Provided" : "✗ Missing"}`,
    );
    doc.moveDown(2);
