...
export default async function handler(
  ...
  try {
    const { principal_id, first_name, middle_name, surname, email } = req.body;
    ...
    if (!response.ok) {
      ...
      return res
        ...
        .json({ error: data?.error || "Failed to submit sub-agent" });
    }
    ...
    try {
      ...
      return res
        .status(202)
        .json({ message: "Sub-agent submitted, email pending", fileUrl });
    }

    res
      .status(200)
      .json({ message: "Sub-agent submitted successfully!", data, fileUrl });
  } catch (error: any) {
